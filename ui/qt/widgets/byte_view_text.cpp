    ...
    line_height_(0)
{
    ...
    //custom addition
    QObject::connect(this, SIGNAL(asciiReady(QString)), parent, SLOT(setAscii(QString)), Qt::UniqueConnection);
    QObject::connect(this, SIGNAL(hexReady(QString)), parent, SLOT(setHex(QString)), Qt::UniqueConnection);
...
void ByteViewText::paintEvent(QPaintEvent *)
{
    ...
    if (!pkt_contents) {
        pkt_contents = true;
        emit asciiReady(pkt_ascii);
        emit hexReady(pkt_hex);
    }
    ...
    if (show_hex_) {
        ...
        for (int tvb_pos = offset; tvb_pos <= max_tvb_pos; tvb_pos++) {
            ...
            case BYTES_HEX:
                ...
                //custom addition
                if (!pkt_contents) {
                    line_no_offset += hexchars[(data_[tvb_pos] & 0xf0) >> 4];
                    line_no_offset += hexchars[data_[tvb_pos] & 0x0f];
                }
                break;
            case BYTES_BITS:
                /* XXX, bitmask */
                for (int j = 7; j >= 0; j--) {
                    ...
                    //custom addition
                    if (!pkt_contents) {
                        line_no_offset += (data_[tvb_pos] & (1 << j)) ? '1' : '0';
                    }
                }
                break;
            }
...
QString ByteViewText::getAscii() {
    return pkt_ascii;
}

QString ByteViewText::getHex() {
    return pkt_hex;
}
