...
void MainWindow::on_actionEditPacketComment_triggered()
{
    ...
    PacketCommentDialog *pc_dialog = new PacketCommentDialog(capture_file_.capFile()->current_frame->num, this, packet_list_->packetComment(), df_combo_box_->currentText(), proto_tree_->getKeypresses(), proto_tree_->getSystemcalls(), byte_view_tab_->getAscii(), byte_view_tab_->getHex(), capture_file_.filePath());
    //pc_dialog.exec();
    pc_dialog->show();
}

/*
    Slot function that stores the current packet comment.
    Used to make multiple similar comments across packets.
*/
void MainWindow::copyCurrentPacketComment(QString comment) {
    //printf("MW: Storing comment\n");
    storedPacketComment = comment;
}

/*
    Slot function the emits the current stored comment.
*/
void MainWindow::pasteStoredComment(guint32 frame) {
    //printf("MW: Sending stored comment\n");
    emit sendStoredComment(frame, storedPacketComment);
}

void MainWindow::handlePCDRejectSignal(guint32 pcdFrame, bool deleteComment) {
    guint32 currentFrame = capture_file_.capFile()->current_frame->num;

    if(deleteComment) {
        //printf("MW: Deleting comment\n");
        if (currentFrame != pcdFrame) {
            packet_list_->goToPacket(pcdFrame);
            packet_list_->setPacketComment(NULL);
            packet_list_->goToPacket(currentFrame);
        }
        else {
            packet_list_->setPacketComment(NULL);
            QModelIndex prevIndex = packet_list_->currentIndex();
            packet_list_->goNextPacket();

            if (packet_list_->currentIndex() == prevIndex) {
...
void MainWindow::handlePCDAcceptSignal(guint32 pcdFrame, QString comment) {
    guint32 currentFrame = capture_file_.capFile()->current_frame->num;

    if (currentFrame != pcdFrame) {
        packet_list_->goToPacket(pcdFrame);
        packet_list_->setPacketComment(comment);
        packet_list_->goToPacket(currentFrame);
    }
    else {
        packet_list_->setPacketComment(comment);
